...
export default class ExplorationScene extends Phaser.Scene {
        ...
        if (entity.currentColor !== 0x00ff00) {
          ...
          if (move) {
            ...
            const occupiedByEntity = this.entities.find(
              e => e !== entity && e.x === move.x && e.y === move.y && !e.isDying
            );
  ...
  private drawDyingEntity(graphics: Phaser.GameObjects.Graphics, entity: Entity): void {
    graphics.clear();
  ...
  private drawDeathTally(): void {
    ...
    const stateCounts: { [key: string]: { growing: number; living: number; dying: number } } = {};
